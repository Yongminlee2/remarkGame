...
| 보스 (5스테이지마다) | 세글자 도깨비(3글자↑) · 받침 지킴이 · 시간 도둑(8초) · 한방 마왕 · 긴말 여왕(4글자↑) · 30↑ 혼합 보스(규칙 2개 조합, 무한) |
...
| 아바타 48종 | 몸 4형태 × 색 6종 × 표정 2세트 조합. 이름 자동 생성("방긋 파랑 네모") |
...
| 상점 | 아바타 + 아이템 5종 |
| 아이템 | ⏰ 시간+15초(60🪙) · 💡 힌트(80🪙) · 🔄 AI 단어 바꾸기(120🪙) · 🎯 2배 획득(120🪙) · 🛡 부활(200🪙) |
...
├── AvatarView.kt         # 몸통+눈2+입 4레이어를 겹쳐 그리는 커스텀 뷰
│
...
app/src/test/java/com/kkeutmal/game/   # 단위 테스트 73개
app/src/main/res/drawable-nodpi/       # Kenney CC0 PNG 33장 (몸통24+얼굴5+아이콘4)
...
- **화면과 로직 분리**: 난이도 곡선·보스 규칙·아바타 카탈로그·미션·보상 계산을 전부 Android 의존 없는
  순수 Kotlin으로 뺐다. 덕분에 위험한 부분(레벨 곡선, 두음법칙, 마이그레이션)을 JUnit 73개로 잡을 수 있고,
  화면은 그 결과를 그리기만 한다.
...
5. **v1.3 — 성장·수집·도전·습관 (대형 확장, 커밋 27개)**
   ...
   - 단위 테스트 도입(JUnit 73개). 순수 로직 파일 8개를 화면과 분리한 덕에
...
- [`docs/specs/2026-07-26-game-expansion-design.md`](docs/specs/2026-07-26-game-expansion-design.md) —
  v1.3 확장 설계: 순환 고리 구조, 스테이지·보스 파라미터, 아바타 등급 배분, 코인 경제, 검증 방법
- [`docs/plans/2026-07-26-game-expansion.md`](docs/plans/2026-07-26-game-expansion.md) —
