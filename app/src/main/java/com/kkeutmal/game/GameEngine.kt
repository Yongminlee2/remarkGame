...
/**
 * 제한시간. 60/45/25/15 로 두 번 손봐서 자리잡은 값이다.
 *
 * 한때 90/60/45/30 으로 늘려 봤다가 되돌렸다. 단계마다 고르게 벌어져 곡선은
 * 매끈했지만 **직접 해 보니 시간이 남아돌아 긴장이 없었다.**
 * 그렇다고 예전 20/12 초는 뒤 두 단계가 너무 빡빡해서, 그 둘만 조금 풀었다.
 *
 * 쉬움에서 보통으로 갈 때 45초가 25초로 떨어지는 낙차가 크긴 해도,
 * 그 낙차가 바로 '난이도를 올렸다'는 체감이라 그대로 둔다.
 */
enum class AiLevel(val label: String, val timerSec: Int, val desc: String) {
    VERY_EASY("매우쉬움", 60, "AI가 이어가기 가장 쉬운 단어만 골라줘요"),
    EASY("쉬움", 45, "AI가 잇기 좋은 단어를 골라줘요"),
    NORMAL("보통", 25, "AI가 무난하게 받아쳐요 (가끔 한방단어!)"),
    HARD("어려움", 15, "AI가 사전 전체로 압박해요 (한방단어 주의!)")
}
